...
echo "Save AE latent (5% data, same forward as 1stage) — multi-GPU"
echo "  Teacher: ${TEACHER_PATH}"
echo "  AE: ${AE_CHECKPOINT}"
echo "  Data: ${DATA_PATH} (data_fraction=${DATA_FRACTION})"
echo "  Save: ${SAVE_DIR}"
echo "  Launch: accelerate (multi-GPU)"
echo ""

accelerate launch "${BASE_PATH}/scripts/AE/kd/save_ae_latent_5pct.py" \
  --data_path "${DATA_PATH}" \
...
echo "Done. Multi-GPU: chunks under save_dir/rank_0/, rank_1/, ... Load with: np.load('.../rank_0/latent_chunk_000000.npz') → z, y_token"
