...
OUTPUT_DIR="${BASE_PATH}/checkpoints/AE/RVQ"

# RVQ Model parameters
NUM_STAGES=${3-25}             # Number of quantization stages (default: 25)
NUM_CODES=${4-1024}            # Codebook size per stage (default: 1024)
...
BATCH_SIZE=${8-128}             # Batch size per GPU (default: 16, total = 16 * 8 = 128)
EPOCHS=${9-1}                 # Number of epochs (default: 10)
LR=${10-1e-4}                  # Learning rate (default: 1e-4)
...
THRESHOLD_EMA_DEAD_CODE=${14-10}  # Dead code threshold (default: 10, recommended: batch_size * 0.5 to batch_size)
...
echo "🚀 Starting RVQ Compressor Training (DDP Optimized Mode)..."
...
echo "     - Save name: best_rvq_s${NUM_STAGES}_c${NUM_CODES}_${G_TAG}_d${COMPRESSED_DIM}_enc${ENCODER_DEPTH}_dec${DECODER_DEPTH}.pt"
...
accelerate launch ${BASE_PATH}/scripts/AE/train/train_RVQ.py \
    --data_path ${DATA_PATH} \
...
echo ""
echo "✅ RVQ Compressor training completed!"
echo "   Output: ${OUTPUT_DIR}"
echo "   Model: best_rvq_s${NUM_STAGES}_c${NUM_CODES}_${G_TAG}_d${COMPRESSED_DIM}_enc${ENCODER_DEPTH}_dec${DECODER_DEPTH}.pt"
