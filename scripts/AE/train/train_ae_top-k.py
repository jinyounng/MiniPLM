...
def compute_logit_losses(
    ...
    """
    Logit KL loss and logit MSE loss.
    If topk is set (e.g. 5000), only the teacher's top-K logits are used; the rest are discarded.
    """
    if topk is None or topk >= teacher_logits_f32.size(-1):
        # Full vocab
        logit_loss = F.kl_div(
        ...
        return logit_loss, logit_mse_loss

    # Top-K: use teacher's top-K indices, gather from both teacher and recon logits
    K = min(int(topk), teacher_logits_f32.size(-1))
    teacher_topk_vals, topk_indices = teacher_logits_f32.topk(K, dim=-1, largest=True, sorted=True)
    recon_topk = torch.gather(recon_logits_f32, dim=-1, index=topk_indices)

    # KL on renormalized distribution over the K tokens
    teacher_probs_topk = F.softmax(teacher_topk_vals / temperature, dim=-1)
    logit_loss = F.kl_div(
        F.log_softmax(recon_topk / temperature, dim=-1),
        teacher_probs_topk,
        reduction="batchmean",
    ) * (temperature ** 2)
    logit_mse_loss = F.mse_loss(recon_topk, teacher_topk_vals)
    return logit_loss, logit_mse_loss
...
def evaluate_model(
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            temperature = 1.0
            logit_loss, logit_mse_loss = compute_logit_losses(
                recon_logits_f32, teacher_logits_f32,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="If set (e.g. 5000), only teacher top-K logits are used for logit loss; rest discarded. None = full vocab.",
    )
