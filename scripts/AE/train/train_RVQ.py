...
class HiddenStateCompressor(nn.Module):
    """
    Complete compression model: Encoder → RVQ → Decoder
    ...
    def __init__(
        ...
        super().__init__()
        # 점진적으로 줄어드는 차원 (depth=3: e.g. [4096, 3072, 2048, 1024])
        enc_dims = np.linspace(teacher_hidden_dim, compressed_dim, encoder_depth + 1).astype(int).tolist()
        enc_layers = []
        ...
        # 점진적으로 늘어나는 차원 — hidden 복원 (depth=3: e.g. [1024, 2048, 3072, 4096])
        dec_dims = np.linspace(compressed_dim, teacher_hidden_dim, decoder_depth + 1).astype(int).tolist()
        dec_layers = []
    ...
    def forward(self, teacher_hidden, return_indices=False, n_q=None, compute_perplexity=False):
        """
        Args:
        ...
        """
        # Encode
        z_e = self.encoder(teacher_hidden)  # [B, compressed_dim]
        ...
        # Decode: hidden 복원 (compressed_dim → teacher_hidden_dim)
        hidden_recon = self.decoder(z_q)  # [B, teacher_hidden_dim]
...
def collate_fn_optimized(batch_indices, dataset, teacher_model, tokenizer, device, max_length, accelerator=None):
    ...
    # Extract last token hidden states and logits
    # Use attention mask to get actual last token
...
def evaluate_model(
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            hidden_recon, info = compressor(h_f32, compute_perplexity=True)
...
def train_compressor_distributed(
    ...
    # Create optimizer BEFORE prepare() - use original model parameters
    ...
    if use_progressive_training:
        # Gradually increase n_q: 25% epochs = 1 stage, 50% = 2 stages, 75% = 3 stages, 100% = all stages
    ...
    for epoch in range(args.epochs):
        ...
        # Compute perplexity periodically (every N steps) instead of every step
        compute_perplexity_period = args.perplexity_compute_period  # 0 = never, N = every N steps
        step_count = 0
        ...
        with tqdm(train_loader, disable=not accelerator.is_main_process, desc=f"Epoch {epoch+1}") as pbar:
            for batch_idx, batch in enumerate(pbar):
                ...
                # Forward: hidden 복원 → teacher lm_head로 logits 얻어서 KL
                hidden_recon, info = compressor(h_f32, n_q=n_q, compute_perplexity=should_compute_perplexity)
                logits = get_lm_logits_from_hidden(teacher_model, hidden_recon)
                kl_loss = F.kl_div(
                    F.log_softmax(logits, dim=-1),
                    F.softmax(teacher_logits, dim=-1),
                ...
                # encodec returns commit_losses as [num_stages] tensor
                # We can weight later stages less (smaller residuals)
                if len(info['commit_losses']) > 0:
                    # Option 1: Simple mean
                    # commit_loss = info['commit_losses'].mean()
                    
                    # Option 2: Weighted by stage (later stages have smaller residuals, so weight less)
                ...
                if accelerator.sync_gradients:
                    # Access model through .module after DDP wrapping
            ...
            if accelerator.is_main_process:
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    save_path = os.path.join(
                        args.output_dir,
                        f"best_rvq_s{args.num_stages}_c{args.num_codes}_{g_tag}_d{args.compressed_dim}_enc{args.encoder_depth}_dec{args.decoder_depth}.pt"
                    )
...
def main():
    parser = argparse.ArgumentParser(description="Train RVQ Compressor using encodec")
    ...
    parser.add_argument("--threshold_ema_dead_code", type=int, default=10, 
                        help="Threshold for dead code expiration. Higher value = less aggressive. "
                             "Recommended: batch_size * 0.5 to batch_size")
