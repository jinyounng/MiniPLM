"""
On-the-fly Conditional AutoEncoder training with Reverse KL.
Same as train_ae_onthefly.py but logit loss uses KL(recon || teacher) instead of KL(teacher || recon).
"""
import os
import argparse
import torch
...
def evaluate_model(
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            # Reverse KL: KL(recon || teacher) - 직접 계산 + clamping (NaN 방지)
            temperature = 1.0
            eps = 1e-8
            recon_probs = F.softmax(recon_logits_f32 / temperature, dim=-1)
            teacher_probs = F.softmax(teacher_logits_f32 / temperature, dim=-1)
            logit_loss = (
                recon_probs * (
                    recon_probs.clamp(min=eps).log() -
                    teacher_probs.clamp(min=eps).log()
                )
        ...
        if val_loader is not None:
            ...
            if accelerator.is_main_process:
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    save_path = os.path.join(args.output_dir, f"best_ae_rkl_ld{args.latent_dim}.pt")
