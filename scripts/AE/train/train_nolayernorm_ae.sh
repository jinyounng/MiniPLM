#!/bin/bash
# Train Conditional AutoEncoder for Knowledge Distillation (No LayerNorm in Decoder)
#
# 이 스크립트는 Teacher 모델의 Hidden States를 압축하는 Conditional AutoEncoder를 학습합니다.
# 디코더의 마지막 LayerNorm이 제거된 버전입니다.
#
# 주요 기능:
...
#
# 사용 예시:
#   bash train_nolayernorm_ae.sh
#   bash train_nolayernorm_ae.sh /path/to/teacher/model
#   bash train_nolayernorm_ae.sh /path/to/teacher/model 25
...
# Data paths
DATA_PATH="/home/jiwonyoon/data1/data/pile_dataset/data_0"  # data_0.bin의 모든 데이터 사용
OUTPUT_DIR="${BASE_PATH}/checkpoints/AE/logit_only/no_layernorm"
...
# TRAIN_SAMPLES는 None으로 설정하여 data_0의 모든 데이터 사용
VAL_SAMPLES=${4-1000}           # Number of validation samples (default: 1000)
...
echo "🚀 Starting AutoEncoder Training (No LayerNorm in Decoder)..."
...
python ${BASE_PATH}/scripts/AE/train/train_nolayernorm_ae.py \
    --data_path ${DATA_PATH} \
